...
function loadData() {
    try {
        ...
        if (decrypted) {
            ...
            if (!dataStore.helpTickets) dataStore.helpTickets = {};
...
app.post('/api/login', loginLimiter, async (req, res) => {
    try {
        ...
        res.json({ success: true, token, user: { id: user.id, name: user.name, isAdmin: user.isAdmin, status: user.status, permissions: user.permissions || [] } });
    } catch (e) {
...
app.get('/api/users', authenticateToken, requireAdmin, (req, res) => {
    const users = Object.values(dataStore.users).map(u => ({ id: u.id, name: u.name, status: u.status, isAdmin: u.isAdmin, permissions: u.permissions || [] }));
...
app.put('/api/users/:userId/permissions', authenticateToken, requireAdmin, (req, res) => {
    ...
    io.emit('permissions:updated', { userId: req.params.userId, permissions: filteredPermissions });
...
app.post('/api/tickets', authenticateToken, (req, res) => {
    try {
        const data = validate(req.body, schemas.createTicket);
        const user = dataStore.users[req.user.userId];
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });
...
app.post('/api/tickets/:id/respond', authenticateToken, (req, res) => {
    try {
        ...
        const data = validate(req.body, schemas.respondToTicket);
        const user = dataStore.users[req.user.userId];
        const message = {
            senderId: user.id,
            senderName: user.name,
        ...
        saveData();
        io.emit('ticket:message', { ticketId: req.params.id, message: message });
