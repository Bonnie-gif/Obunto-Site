...
function loadData() { var s = localStorage.getItem('vintageSiteDatav17'); if(s) try { data = JSON.parse(s); } catch(e){} render(); }
function saveData() { localStorage.setItem('vintageSiteDatav17', JSON.stringify(data)); }

// Função auxiliar para sanitização segura
function safeHTML(content) {
    if (window.DOMPurify) {
        return DOMPurify.sanitize(content);
    }
    // Fallback básico se DOMPurify não carregar
...
// Renderização segura de links (Sem innerHTML concatenado)
function renderLinks() {
    ...
    data.links.items.forEach(function(l) {
        ...
        a.textContent = l.icon || '?'; // textContent evita XSS no ícone
...
function renderGallery() { 
    ...
    data.gallery.forEach(function(g, i){ 
        ...
        if(g.data) {
            ...
            img.src = g.data; // Base64 data URI is generally safe if generated by FileReader
...
function saveWelcome() { 
    ...
    data.welcome.text= safeHTML($('inWTxt').innerHTML); 
...
function savePromo() { 
    ...
    data.promo.text= safeHTML($('inPTxt').innerHTML); 
    saveData(); render(); closeModal(); 
}
...
function saveTos(){ 
    data.tos = safeHTML($('inTos').innerHTML); 
    saveData(); render(); closeModal(); 
}
